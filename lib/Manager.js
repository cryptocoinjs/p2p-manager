...
// TODO: make input a Host object rather than two arguments of host and port.
Manager.prototype.addPool = function addPool(host, port) {
	if (this.activePeerCount < this.options.minPeers) {
	  return this.addActive(host, port);
	}
	// TODO: enable adding multiple hosts at once, so de-duplicate doesn't run every time
	this.poolPeers.push({host: host, port: port});
...
Manager.prototype.addActive = function addActive(host, port) {
	var p = this.connect(host, port);
...
Manager.prototype.connect = function connect(host, port) {
  ...
  p.once('end', function(d) {
    ...
    if (typeof self.handleEnd === 'function') {
		...
		if (d.peer.state !== 'disconnecting') self.disconnect(d.peer); // Other end hung up on us; no need to hang around
  });
  p.on('error', function(d) {
    ...
    if (typeof self.handleError === 'function') {
		...
		if (d.peer.state !== 'disconnecting') self.disconnect(d.peer); // Close the connection that errored
  });
...
Manager.prototype.shutdown = function shutdown() {
	...
	for (var uuid in this.activePeers) {
		if (this.activePeers.hasOwnProperty(uuid) && this.activePeers[uuid] instanceof Peer) {
			...
			this.disconnect(this.activePeers[uuid]);
		}
	}
};
